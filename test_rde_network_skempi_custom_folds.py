...
from rde.utils.skempi_custom_folds import SkempiCustomFoldsDatasetManager, per_complex_corr
...
if __name__ == '__main__':
    ...
    parser.add_argument('--output', type=str, default='./results_skempi_custom_folds.csv')
    ...
    with torch.no_grad():
        for fold in range(args.num_cvfolds):
            ...
            model.eval()
            for i, batch in enumerate(tqdm(dataset_mgr.get_val_loader(fold), desc=f'Fold {fold}', dynamic_ncols=True)):
                # Prepare data
