...
def load_custom_fold_splits(folds_dir, threshold, fold_index):
    ...
    Returns:
        train_complex_ids: Set of complex IDs for training
        test_complex_ids: Set of complex IDs for testing
    """
...
def split_table_wildtype_only(table, folds_dir, threshold, num_folds=3, fold_id=0, seed=2022):
    """Split table using the wildtype-only cross-validation folds."""
    if num_folds == 1:
        return table, table, table
    assert fold_id < num_folds

    # Load fold splits (complex IDs like '0_1CSE')
    train_complex_ids, test_complex_ids = load_custom_fold_splits(folds_dir, threshold, fold_id)

    print(f"DEBUG: Found {len(train_complex_ids)} train complexes and {len(test_complex_ids)} test complexes")
    print(f"DEBUG: Total table has {len(table)} rows")
    print(f"DEBUG: Sample train complex IDs: {list(train_complex_ids)[:5]}")
    print(f"DEBUG: Sample test complex IDs: {list(test_complex_ids)[:5]}")
    print(f"DEBUG: Sample table pdbcode: {table['pdbcode'].unique()[:5]}")

    # Extract pdbcode from fold complex ids (take token after underscore)
    def ids_to_pdbcodes(ids):
        pdbs = []
        for cid in ids:
            parts = cid.split('_')
            if len(parts) >= 2:
    ...
    train_pdb_codes = ids_to_pdbcodes(train_complex_ids)
    test_pdb_codes = ids_to_pdbcodes(test_complex_ids)

    print(f"DEBUG: Extracted train PDB codes: {list(train_pdb_codes)[:5]}")
    print(f"DEBUG: Extracted test PDB codes: {list(test_pdb_codes)[:5]}")

    # Masks by pdbcode
    train_mask_all = table['pdbcode'].isin(train_pdb_codes)
    test_mask = table['pdbcode'].isin(test_pdb_codes)
    print(f"DEBUG: Rows with train pdbcodes: {int(train_mask_all.sum())}")
    print(f"DEBUG: Rows with test pdbcodes: {int(test_mask.sum())}")

    # Split train pdbcodes into train/val (80/20) for calibration
    train_pdb_list = sorted(list(train_pdb_codes))
    random.Random(seed).shuffle(train_pdb_list)
    split_idx = max(1, int(0.8 * len(train_pdb_list))) if len(train_pdb_list) > 1 else 1
    train_pdb_final = set(train_pdb_list[:split_idx])
    val_pdb = set(train_pdb_list[split_idx:])

    train_mask = table['pdbcode'].isin(train_pdb_final)
    val_mask = table['pdbcode'].isin(val_pdb)

    train_table = table[train_mask]
    val_table = table[val_mask]
    test_table = table[test_mask]

    print(f"DEBUG: Final split - Train rows: {len(train_table)}, Val rows: {len(val_table)}, Test rows: {len(test_table)}")
...
def fit(table_train, table_val, table_test, device, iters, alternate=None):
    ...
    print(f"DEBUG: Input table sizes - Train: {len(table_train)}, Val: {len(table_val)}, Test: {len(table_test)}")
    ...
    print(f"DEBUG: Data shapes - Train ddG: {ddG_train.shape}, Val ddG: {ddG_val.shape}, Test ddG: {ddG_test.shape}")
    ...
    for i in range(1, iters+1):
        ...
        if i == 1:
            print(f"DEBUG: First iteration - ddG_train stats: min={ddG_train.min().item():.4f}, max={ddG_train.max().item():.4f}, mean={ddG_train.mean().item():.4f}")
            print(f"DEBUG: First iteration - Hs_train shapes: {[H.shape for H in Hs_train]}")
            print(f"DEBUG: First iteration - Ts_train shapes: {[T.shape for T in Ts_train]}")
        ...
        if i == 1:
            print(f"DEBUG: First iteration - y_pred stats: min={y_pred.min().item():.4f}, max={y_pred.max().item():.4f}, mean={y_pred.mean().item():.4f}")
            print(f"DEBUG: First iteration - loss: {loss.item():.4f}")
        ...
        if i % 100 == 0:
            with torch.no_grad():
                ...
                # Handle case where no groups meet the threshold (same as per_complex_corr)
                if len(df_corr) == 0:
                    pearson_g, spearman_g = 0.0, 0.0
                else:
                    corr_mean = df_corr[['pearson', 'spearman']].mean()
                    pearson_g, spearman_g = corr_mean['pearson'], corr_mean['spearman']
...
def run_calibration(
    ...
    output_dir='./RDE_linear_skempi_wildtype_only',
    single=False,
):
    seed_all(seed)
    ...
    for i, seed in enumerate(seeds):
        for fold_id in range(num_folds):
            ...
            table_train, table_val, table_test = split_table_wildtype_only(
                table, 
                folds_dir,
                threshold,
                num_folds=num_folds,
...
if __name__ == '__main__':
    ...
    parser.add_argument('--threshold', type=int, default=45, help='Percentage threshold for fold splits (e.g., 45 for 45_percent)')
    parser.add_argument('-o', '--output_dir', type=str, default='./RDE_linear_skempi_wildtype_only')
    ...
    parser.add_argument('-n', '--num_folds', type=int, default=10)
