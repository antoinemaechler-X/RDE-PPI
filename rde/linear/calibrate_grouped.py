...
def split_table_grouped(table, grouped_csv_path, num_folds=3, fold_id=0, seed=2022):
    """Split table by complex groups instead of individual complexes."""
    if num_folds == 1:
        return table, table, table
    assert fold_id < num_folds
    
    # Create mapping from PDB codes to group IDs
    pdb_to_group = create_complex_to_group_mapping(grouped_csv_path)
    
    # Add group_id column to table
    table['group_id'] = table['pdbcode'].map(pdb_to_group)
    
    # Remove rows where pdbcode is not in the grouped file
    table = table.dropna(subset=['group_id'])
    
    # Get unique group IDs and shuffle
    group_ids = table['group_id'].unique()
    random.Random(seed).shuffle(group_ids)

    fold_size = math.ceil(len(group_ids) / num_folds)
    train_groups, val_groups, test_groups = [], [], []
    for i in range(num_folds):
        fold = group_ids[i * fold_size: (i + 1) * fold_size]
        if i == fold_id:
            train_groups.extend(fold)
        elif i == (fold_id+1) % num_folds:
            val_groups.extend(fold)
        else:
            test_groups.extend(fold)

    train_mask = table['group_id'].isin(train_groups)
    val_mask = table['group_id'].isin(val_groups)
    test_mask = table['group_id'].isin(test_groups)
    
    # Remove group_id column before returning
    table = table.drop(columns=['group_id'])
...
def run_calibration(
    ...
    for i, seed in enumerate(seeds):
        for fold_id in range(num_folds):
            ...
            table_train, table_val, table_test = split_table_grouped(
                table, 
                grouped_csv_path,
                seed=seed, 
                num_folds=num_folds,
...
if __name__ == '__main__':
    ...
    parser.add_argument('-o', '--output_dir', type=str, default='./RDE_linear_skempi_grouped')
