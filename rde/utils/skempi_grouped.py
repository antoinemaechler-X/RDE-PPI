...
from rde.datasets import SkempiGroupedDataset
...
class SkempiGroupedDatasetManager(object):
    ...
    def init_loaders(self, fold):
        ...
        # Count unique groups in train and val datasets
        train_groups = set()
        val_groups = set()
        
        # Get the grouped CSV to map complexes to groups
        import pandas as pd
        grouped_df = pd.read_csv(config.data.grouped_csv_path)
        pdb_to_group = {}
        ...
        # Count groups in train dataset
        for entry in train_dataset.entries:
            pdbcode = entry['pdbcode']
            if pdbcode in pdb_to_group:
                train_groups.add(pdb_to_group[pdbcode])
        
        # Count groups in val dataset
        for entry in val_dataset.entries:
            pdbcode = entry['pdbcode']
            if pdbcode in pdb_to_group:
                val_groups.add(pdb_to_group[pdbcode])
        
        self.logger.info('Fold %d: %d train groups, %d test groups' % (fold, len(train_groups), len(val_groups)))
        self.logger.info('      %d train samples, %d test samples' % (len(train_dataset), len(val_dataset)))
