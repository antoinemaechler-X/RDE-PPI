...
class DDG_RDE_Network_SeparateStructures(nn.Module):
    ...
    def forward(self, batch):
        # Check if we have nested structure (separate structures) or flat structure (regular)
        if 'wildtype' in batch and 'mutant' in batch:
            # Separate structures approach
            batch_wt = batch['wildtype']
            batch_mt = batch['mutant']

            # Encode wildtype structure
            h_wt = self.encode(batch_wt)
            
            # Encode mutant structure
            ...
            # Use ddG from wildtype batch (should be the same in both)
            ddg_true = batch_wt['ddG']
        else:
            # Regular approach (flat batch structure)
        ...
        loss = (F.mse_loss(ddg_pred, ddg_true) + F.mse_loss(ddg_pred_inv, -ddg_true)) / 2
