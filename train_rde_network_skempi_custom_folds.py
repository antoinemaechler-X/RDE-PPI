...
if __name__ == '__main__':
    ...
    parser.add_argument('--logdir', type=str, default='./logs_skempi_custom_folds')
    ...
    if args.debug:
        ...
        if args.resume:
            log_dir = get_new_log_dir(args.logdir, prefix='%s(%d-%d)-resume' % (config_name, args.num_cvfolds, args.threshold), tag=args.tag)
        else:
            log_dir = get_new_log_dir(args.logdir, prefix='%s(%d-%d)' % (config_name, args.num_cvfolds, args.threshold), tag=args.tag)
        ...
        for it in range(it_first, config.train.max_iters + 1):
            ...
            if it % config.train.val_freq == 0:
                ...
                if not args.debug:
                    ...
                    prev_ckpt = os.path.join(ckpt_dir, '%d.pt' % (it - config.train.val_freq))
