...
from rde.utils.skempi_grouped import SkempiGroupedDatasetManager, per_complex_corr
...
if __name__ == '__main__':
    ...
    parser.add_argument('--output', type=str, default='./results_skempi_grouped.csv')
    ...
    dataset_mgr = SkempiGroupedDatasetManager(
        config, 
    ...
    with torch.no_grad():
        for fold in range(args.num_cvfolds):
            ...
            model.eval()
            for i, batch in enumerate(tqdm(dataset_mgr.get_val_loader(fold), desc=f'Test fold {fold}', dynamic_ncols=True)):
                # Prepare data
    ...
    for fold in range(args.num_cvfolds):
        ...
        if len(fold_results) > 0:
            fold_pearson = fold_results[['ddG', 'ddG_pred']].corr('pearson').iloc[0, 1]
            fold_spearman = fold_results[['ddG', 'ddG_pred']].corr('spearman').iloc[0, 1]
            logger.info(f'Fold {fold}: Pearson {fold_pearson:.6f} Spearman {fold_spearman:.6f} ({len(fold_results)} samples)') 